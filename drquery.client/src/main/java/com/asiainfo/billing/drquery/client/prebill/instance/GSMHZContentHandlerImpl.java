 ...
 * @author daibei
 */
public class GSMHZContentHandlerImpl{
        ...
        for (int i = 0; i < contentSize; i++) {
            ...
            for (int j = 0; j < fieldsSize; j++) {
                ...
                if(j == 5) {
                    ...
                    total5 += PreBillDRQueryUtil.formatNumber(input)/60;
                } else if(j == 15) {
                    singleContent.add(input);
                    singleContent.add("");
                } else if(j == 18) {
                    singleContent.add(input);
                    singleContent.add(PreBillDRQueryUtil.formatMoney((String) singleRow.get(10)));//信息费
                } else if(j == 21) {
                    singleContent.add(PreBillDRQueryUtil.formatNumber(input)/60);
                    total6 += PreBillDRQueryUtil.formatNumber(input)/60;//免费分钟数2
                } else if(j == 23) {
                    singleContent.add(PreBillDRQueryUtil.formatNumber(input)/60);
                    total7 += PreBillDRQueryUtil.formatNumber(input)/60;//免费分钟数3
                } else {
                    singleContent.add(input);
                }
            }
