...
//		}
	...
	public static void putPageStartKeyOrCountToCache(String key, Map<String, List<Map<String, String>>> recordInfo,
        ...
        ((RedisCache)redisCache).getRedisTemplate().opsForHash().put(key,nextStartIndex,info);
		((RedisCache)redisCache).getRedisTemplate().expire(key, timeout, TimeUnit.SECONDS);
//        dump(key);
    ...
    public static void put(String key, Object hashKey, Object value, int timeout) {
        ((RedisCache)redisCache).getRedisTemplate().opsForHash().put(key, hashKey, value);
        if(timeout != 0) {
            ((RedisCache)redisCache).getRedisTemplate().expire(key, timeout, TimeUnit.SECONDS);
        }
    }
